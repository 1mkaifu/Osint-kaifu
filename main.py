...
# ===== CONFIG =====
BOT_TOKEN = os.getenv("8234235978:AAEaivd39C5FSrTtjZeEz1JPQflOzC-Js-A")  # env se token lena
...
def check_balance_and_warn(context, chat_id, u):
    if u["credits"] <= 0:
        button = InlineKeyboardMarkup(
            [[InlineKeyboardButton("💳 Recharge Now", url="https://t.me/IG_BANZ")]]
        )
        send_with_photo(
            context,
            chat_id,
            "⚠️ Your balance is 0 credits.\nPlease recharge to continue."
            + COPYRIGHT,
            reply_markup=button,
        )
        return False
        ...
        send_with_photo(
            context,
            chat_id,
            "⚠️ Low Balance: Only 1 credit left!\nRecharge soon." + COPYRIGHT,
        )
    return True
...
def phone(update, context):
    ...
    if not context.args:
        ...
        return
    if not check_balance_and_warn(context, chat_id, u):
        return

    number = context.args[0]
    r = requests.get(
        f"https://api.elitepredator.app/search_mobile?mobile={number}"
    ).json()
    u["credits"] -= 1
    u["searches"] += 1
    save_users()
...
def pincode(update, context):
    ...
    code = context.args[0]
    r = requests.get(
        f"https://pincode-info-j4tnx.vercel.app/pincode={code}"
    ).json()
    u["credits"] -= 1
    u["searches"] += 1
    save_users()
...
def balance(update, context):
    ...
    u = ensure_user(chat_id)
    send_with_photo(
        context, chat_id, f"💳 Balance: {u['credits']} credits" + COPYRIGHT
    )
    ...
    def wrapper(update, context):
        if update.message.chat_id not in ADMINS:
            send_with_photo(
                ...
                "Chala ja Randike Maa chod di jayegi banwau tujhe admin ka choda",
            )
            return
...
def addcredits(update, context):
    ...
    send_with_photo(
        context,
        update.message.chat_id,
        f"✅ Added {amt} credits to {uid}" + COPYRIGHT,
    )
...
def subcredits(update, context):
    ...
    uid, amt = str(uid), int(amt)
    u = ensure_user(uid)
    u["credits"] = max(0, u["credits"] - amt)
    save_users()
    send_with_photo(
        context,
        update.message.chat_id,
        f"✅ Subtracted {amt} credits from {uid}" + COPYRIGHT,
    )
...
def exportusers(update, context):
    with open("users.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["UserID", "Credits", "Joined", "Searches"])
        for uid, d in USERS.items():
...
def broadcast(update, context):
    ...
    for uid in USERS.keys():
        try:
            send_with_photo(
                context, int(uid), f"📢 Broadcast:\n{msg}" + COPYRIGHT
            )
        except:
            pass
    send_with_photo(
        context, update.message.chat_id, "✅ Broadcast sent." + COPYRIGHT
    )
